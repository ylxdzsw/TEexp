...
let solve_thresh (topo:topology) (_:demands) budget : scheme =
  (* Not used *)
  ...
  let new_scheme =
    if not (SrcDstMap.is_empty !prev_scheme) then !prev_scheme
      ...
      let all_ksp =
        all_pair_k_shortest_path topo budget host_set in
      let thresh = 1.2 in
      ...
      SrcDstMap.fold apmsp ~init: SrcDstMap.empty
        ~f:(fun ~key:(u, v) ~data:(_,n,probs) acc ->
          if (VertexSet.mem host_set u) && (VertexSet.mem host_set v) then
            let paths = get_all_shortest_paths u v topo apmsp budget in
